...
import {
  getCategories,
  getSingleCategory,
} from "@/app/rtk/slices/categoriesSlice";
...
export default function AddCategory({
  ...
  const { categories, singleCategory } = useSelector(
      ...
      <div className="flex md:flex-row flex-col items-center justify-between mb-3">
        <div className="md:w-[48%] w-full">
          ...
          {formErrors.category_id && (
            <div className="text-red-500 text-sm">{formErrors.category_id}</div>
          )}
        </div>
        <div className="md:w-[48%] w-full">
          <CustomSelectOptions
            ...
            label="Sub Category"
            hasError={Boolean(formErrors.sub_category_id)}
            options={singleCategory?.subcategories || []}
          />
          {formErrors.sub_category_id && (
            <div className="text-red-500 text-sm">{formErrors.sub_category_id}</div>
          )}
        </div>
      </div>
      <div className="flex md:flex-row flex-col items-center justify-between mb-3">
        <div className="md:w-[48%] w-full">
          <CustomSelectOptions
            ...
            id="brand_id"
            hasError={Boolean(formErrors.brand_id)}
            label="Brands"
            options={brands}
          />
          {formErrors.brand_id && (
            <div className="text-red-500 text-sm">{formErrors.brand_id}</div>
          )}
        </div>
        <div className="md:w-[48%] w-full">
          <CustomSelectOptions
            ...
            onChange={formChangeEvent}
            hasError={Boolean(formErrors.unit_of_measure_id)}
            label="Units of Measure"
            options={units}
          />
          {formErrors.unit_of_measure_id && (
            <div className="text-red-500 text-sm">{formErrors.unit_of_measure_id}</div>
          )}
        </div>
      </div>
    </div>
