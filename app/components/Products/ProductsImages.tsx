...
export default function ProductsImages({
  ...
  const renderedImages = images.map((src: string) =>
    URL.createObjectURL(src as unknown as File)
  );
  ...
  return (
    <div className="mt-5 p-6 border-[1px] bg-white rounded-lg shadow-sm border-gray_dark">
      <h5 className="text-lg font-bold max-w-full mb-6">
        ...
        {formErrors.images && (
          <span className="text-red-500 font-normal block text-sm">
            {formErrors.images as string}
          </span>
        )}
      </h5>
      ...
      <div className="max-w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2">
        {renderedImages.map((src: string, index: number) => (
          <div
            key={index}
            className="p-3 bg-slate-100 rounded-lg border border-gray_dark relative"
          >
